...
class GradientDescendent(Optimizer):
    ...
    def update(self, layer:LayerMetaClass, t:int=1) -> None:
...
class Adam(Optimizer):
    ...
    def update(self, layer:LayerMetaClass, t:int=1) -> Tuple[np.array, np.array]:
        ...
        layer.momentum_weights = self.beta1*layer.momentum_weights + ((1-self.beta1)*layer.d_W)
        layer.momentum_bias = self.beta1*layer.momentum_bias + ((1-self.beta1)*layer.d_B)
        layer.velocity_weights = self.beta2*layer.velocity_weights + ((1-self.beta2)*(layer.d_W**2))
        ...
        weights = layer.weights.T - self.lr*(momentum_weights_corr/(np.sqrt(velocity_weights_corr)+self.epsilon))
        bias = layer.bias - self.lr*(momentum_bias_corr/(np.sqrt(velocity_bias_corr)+self.epsilon))
